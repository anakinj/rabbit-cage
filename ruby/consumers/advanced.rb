...
dead.bind(dlx, routing_key: '*')

logger.info('Subscribing to queue messages')

def handler(name, _routing_key, payload)
  logger.info "#{'%16s' % name} | '#{payload}'"
end

black.subscribe do |delivery_info, _properties, payload|
  handler('BLACK', delivery_info.routing_key, payload)
end

red.subscribe do |delivery_info, _properties, payload|
  handler('RED', delivery_info.routing_key, payload)
end

all_colors.subscribe do |delivery_info, _properties, payload|
  handler('ALL-COLOR', delivery_info.routing_key, payload)
end

nack.subscribe(manual_ack: true) do |delivery_info, _properties, payload|
  handler('NACK', delivery_info.routing_key, payload)
  delivery_info.channel.reject(delivery_info.delivery_tag)
end

dead.subscribe do |delivery_info, _properties, payload|
